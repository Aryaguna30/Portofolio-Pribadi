        ...
        <div class="form-layout">
          ...
          <div class="form-sidebar">
            <div class="sidebar-card">
              ...
              <p class="hint">Kosongkan jika tidak ingin mengganti thumbnail.</p>
            </div>
...
function parseTechStack(ts) {
  if (!ts) return [''];
  if (Array.isArray(ts)) return ts.length ? ts : [''];
  try { const arr = JSON.parse(ts); return arr.length ? arr : ['']; } catch { return ['']; }
}

const form = useForm({
  title: props.project.title ?? '',
  description: props.project.description ?? '',
  tech_stack: parseTechStack(props.project.tech_stack),
...
function addTech() { form.tech_stack.push(''); }
function removeTech(idx) { form.tech_stack.splice(idx, 1); }
...
.sidebar-card__title { font-size: 0.875rem; font-weight: 700; color: var(--text); }
...
.btn--primary { background: linear-gradient(135deg, var(--accent, #7c3aed), #4f46e5); color: #fff; }
